...
function buildModels(config: ModelConfig): {
  ...
  if (config.provider_type === "builtin") {
    ...
    const customBaseUrlModelOverrides = isCustomResponsesEndpoint(
      ...
      : {};
    const keylessSession =
      config.auth_method === "ambient" &&
      !config.api_key &&
      (config.pi_provider_id === "opencode" || config.pi_provider_id === "opencode-go");
    const effectiveProvider = keylessSession
      ? {
          ...provider,
          auth: {
            ...provider.auth,
            apiKey: {
              ...
              resolve: async () => ({ auth: {}, source: "keyless session" }),
            },
          },
        }
      : provider;
    ...
    models.setProvider(effectiveProvider);
    const model = {
      ...modelTemplate,
      ...(builtinModel
        ? {}
