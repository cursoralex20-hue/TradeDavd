...
data class PiProviderDefinition(
    ...
    val supportsKeylessSession: Boolean = false,
)

object PiProviderSession {
    const val OpenCodeZenBaseUrl = "https://opencode.ai/zen/v1"
    ...
    fun keylessSessionHeaders(): Map<String, String> {
        val session = keylessSessionHeaderValue()
        return mapOf(
            OpenCodeZenSessionHeader to session,
            OpenCodeZenAuthHeader to session,
        )
    }
}
...
object PiProviderCatalog {
    val providers: List<PiProviderDefinition> = listOf(
        ...
        builtin("opencode", "OpenCode Zen", "https://opencode.ai/zen/v1", "mimo-v2.5-free", supportsAmbientAuth = true, supportsKeylessSession = true, category = "Recommended"),
        builtin("opencode-go", "OpenCode Zen Go", "https://opencode.ai/zen/v1", "deepseek-v4-flash-free", category = "Coding", supportsKeylessSession = true),
...
fun PiProviderDefinition.seededModelIds(): List<String> = when {
    supportsKeylessSession && PiProviderSession.OpenCodeZenFreeModels.contains(defaultModelId) ->
        PiProviderSession.OpenCodeZenFreeModels
    defaultModelId.isNotBlank() -> listOf(defaultModelId)
    else -> emptyList()
}
...
fun PiProviderDefinition.defaultAuthMethod(): ProviderAuthMethod = when {
    supportsKeylessSession -> ProviderAuthMethod.Ambient
...
private fun builtin(
    ...
    supportsKeylessSession: Boolean = false,
    category: String,
    ...
    category = category,
    supportsKeylessSession = supportsKeylessSession,
)
